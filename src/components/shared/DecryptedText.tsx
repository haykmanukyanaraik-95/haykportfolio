// Эффект расшифровки текста — ReactBits (оригинал без изменений)
"use client";
...
export default function DecryptedText({
        ...
        if (sequential) {
          ...
          if (direction === 'reverse') {
            if (pointerRef.current < orderRef.current.length) {
              ...
              setDisplayText(shuffleText(text, newRevealed));
              if (newRevealed.size === 0) { clearInterval(interval); setIsAnimating(false); setIsDecrypted(false); }
              return newRevealed;
            } else {
              clearInterval(interval); setIsAnimating(false); setIsDecrypted(false);
              return prevRevealed;
            }
          }
        } else {
  ...
  return (
    <motion.span
      ref={containerRef}
      className={`inline whitespace-pre-wrap ${parentClassName}`}
      {...animateProps}
      {...props}
    >
